...
def send_int(t1: Place, send_socket: socket.socket, to_send: int) -> Place:
    IOExists1(Place)(lambda t_post: (
        ...
        Exsures(ConnectionRefusedError, Result() is t1 and token(t1, 1)),
        Exsures(ConnectionRefusedError, UDP_send_int(t1, send_socket.getpeername()[0], send_socket.getpeername()[1], to_send, t_post)),
...
def receive_int(t1: Place, rec_socket: socket.socket) -> Tuple[Place, int]:
    IOExists2(int, Place)(lambda res, t_post: (
        ...
        Exsures(socket.timeout, Result()[0] is t1 and res is None and token(t1, 1)),
        Exsures(socket.timeout, UDP_receive_int(t1, rec_socket.getsockname()[1], res, t_post)),
    ))
